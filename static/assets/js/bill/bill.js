...
var purchased_price;
async function products(organization="all",product_id="all")
{
    // product_id="all";

    url='/products/'+organization+"/"+product_id+'/';
    ...
    console.log("##2 products response product",response)
        ...
        product_data=data;
        for(key in data){
            selling_price_obj[data[key]['id']]=data[key]['product_price']['selling_price']
            purchasing_price_obj[data[key]['id']]=data[key]['product_price']['purchased_price']
            id=data[key]['id'];
            if(item_in_list_select_index===0)
            {
            selling_price=data[key]['product_price']['selling_price'];
            purchased_price=data[key]['product_price']['purchased_price']
            }
...
{
    console.log("insert_selling_price");
...
{
    console.log("change_price_field");
...
async function adding_row(){
   ...
   if(bill_type.value=="PURCHASE" || bill_type.value=="RECEIVEMENT")
    ...
    for(key in product_data){
        ...
        selling_price_obj[product_data[key]['id']]=product_data[key]['product_price']['selling_price']
        purchasing_price_obj[product_data[key]['id']]=product_data[key]['product_price']['purchased_price']
        id=product_data[key]['id'];
        if(item_in_list_select_index===0)
        {
        selling_price=product_data[key]['product_price']['selling_price'];
        purchased_price=product_data[key]['product_price']['purchased_price'];
        }
        item_in_list_select_index=item_in_list_select_index+1;
    }
    ...
    {
        //price of the bill_receiver organizaiton product 
...
function total_and_paid_validation()
{
    ...
    console.log("total_and_paid_validation");
...
try{
     document.getElementById("bill").addEventListener("submit",async function(e){
        ...
        organization=document.getElementById("organization");
        bill_rcvr_org=document.getElementById("bill_rcvr_org");
        bill_receiver_store=document.getElementById("bill_receiver_store");
        ...
        if(bill_type.value=="PAYMENT" || bill_type.value=="RECEIVEMENT"){
            bill_obj={
                ...
                "bill_receiver_store":bill_receiver_store.value,
